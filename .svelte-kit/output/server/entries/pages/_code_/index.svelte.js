import { g as getContext, n as noop$1, b as safe_not_equal, c as create_ssr_component, d as subscribe, f as each, e as escape, a as add_attribute, h as assign, i as now, l as loop, j as identity$1, v as validate_component, k as globals, o as spread, p as escape_object, q as escape_attribute_value, m as missing_component, r as add_classes, t as createEventDispatcher } from "../../../chunks/index-cf0e86af.js";
import { a as assets, b as base } from "../../../chunks/paths-6758d194.js";
...
import { Adder, merge, extent as extent$1 } from "d3-array";
import "topojson";
...
const Axis = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$unsubscribe_x_origin = subscribe(x_origin, (value) => $x_origin = value);
  $$unsubscribe_y_origin = subscribe(y_origin, (value) => $y_origin = value);
  $$unsubscribe_x_offset = subscribe(x_offset, (value) => $x_offset = value);
  $$unsubscribe_y_offset = subscribe(y_offset, (value) => $y_offset = value);
  ...
  return `${each(Array(50), (item, i) => {
    return `${$x_origin ? `
  <g transform="${"translate(" + escape($x_origin + $x_offset * (i - 25)) + ",0 )"}"><line x1="${"0"}" x2="${"0"}" y1="${"5000"}" y2="${"-5000"}"${add_attribute("stroke", i == 25 ? "purple" : "lightblue", 0)} class="${"svelte-1ma9czi"}"></line></g>


  <g transform="${"translate(" + escape($x_origin) + ",\n" + escape($y_origin - $y_offset * (i - 25)) + " )"}"><line x1="${"-5000"}" x2="${"2000"}" y1="${"-0"}" y2="${"0"}"${add_attribute("stroke", i == 25 ? "purple" : "lightblue", 0)} class="${"svelte-1ma9czi"}"></line></g>` : ``}`;
  })}
${chart_key ? `<g transform="${"translate(" + escape(padding) + "," + escape(height - padding * 2 + 25) + ")"}">${values.length ? `${each(values, (value, i) => {
...
function circleStream(stream, radius, delta, direction, t0, t1) {
  ...
  for (var point, t = t0; direction > 0 ? t > t1 : t < t1; t -= step2) {
...
function clipCircle(radius) {
  ...
  function intersect(a2, b2, two) {
    ...
    if (!two)
      return q;
    var lambda0 = a2[0], lambda1 = b2[0], phi0 = a2[1], phi1 = b2[1], z;
    if (lambda1 < lambda0)
...
function clipLine(a2, b2, x02, y02, x12, y12) {
  var ax = a2[0], ay = a2[1], bx = b2[0], by = b2[1], t0 = 0, t1 = 1, dx = bx - ax, dy = by - ay, r;
  r = x02 - ax;
  if (!dx && r > 0)
    return;
  r /= dx;
...
function centroidPointLine(x2, y2) {
  var dx = x2 - x0$1, dy = y2 - y0$1, z = sqrt(dx * dx + dy * dy);
  X1 += z * (x0$1 + x2) / 2;
  Y1 += z * (y0$1 + y2) / 2;
  Z1 += z;
  centroidPoint(x0$1 = x2, y0$1 = y2);
...
function fitExtent(projection2, extent2, object) {
  return fit(projection2, function(b2) {
    var w = extent2[1][0] - extent2[0][0], h = extent2[1][1] - extent2[0][1], k2 = Math.min(w / (b2[1][0] - b2[0][0]), h / (b2[1][1] - b2[0][1])), x2 = +extent2[0][0] + (w - k2 * (b2[1][0] + b2[0][0])) / 2, y2 = +extent2[0][1] + (h - k2 * (b2[1][1] + b2[0][1])) / 2;
...
function fitWidth(projection2, width2, object) {
  return fit(projection2, function(b2) {
    var w = +width2, k2 = w / (b2[1][0] - b2[0][0]), x2 = (w - k2 * (b2[1][0] + b2[0][0])) / 2, y2 = -k2 * b2[0][1];
    projection2.scale(150 * k2).translate([x2, y2]);
  }, object);
}
...
function resample$1(project, delta2) {
  function resampleLineTo(x02, y02, lambda0, a0, b0, c0, x12, y12, lambda1, a1, b1, c1, depth, stream) {
    ...
    if (d2 > 4 * delta2 && depth--) {
      var a2 = a0 + a1, b2 = b0 + b1, c = c0 + c1, m = sqrt(a2 * a2 + b2 * b2 + c * c), phi2 = asin(c /= m), lambda2 = abs(abs(c) - 1) < epsilon || abs(lambda0 - lambda1) < epsilon ? (lambda0 + lambda1) / 2 : atan2(b2, a2), p = project(lambda2, phi2), x2 = p[0], y2 = p[1], dx2 = x2 - x02, dy2 = y2 - y02, dz = dy * dx2 - dx * dy2;
      if (dz * dz / d2 > delta2 || abs((dx * dx2 + dy * dy2) / d2 - 0.5) > 0.3 || a0 * a1 + b0 * b1 + c0 * c1 < cosMinDistance) {
        resampleLineTo(x02, y02, lambda0, a0, b0, c0, x2, y2, lambda2, a2 /= m, b2 /= m, c, depth, stream);
        stream.point(x2, y2);
        resampleLineTo(x2, y2, lambda2, a2, b2, c, x12, y12, lambda1, a1, b1, c1, depth, stream);
      }
    }
  }
  return function(stream) {
...
function projectionMutator(projectAt) {
  var project, k2 = 150, x2 = 480, y2 = 250, lambda = 0, phi = 0, deltaLambda = 0, deltaPhi = 0, deltaGamma = 0, rotate2, alpha = 0, sx = 1, sy = 1, theta = null, preclip = clipAntimeridian, x02 = null, y02, x12, y12, postclip = identity, delta2 = 0.5, projectResample, projectTransform, projectRotateTransform, cache, cacheStream;
  ...
  projection2.fitWidth = function(width2, object) {
    return fitWidth(projection2, width2, object);
  };
...
const App$2 = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  let points = [];
  let zeds = tweened([]);
  $$unsubscribe_zeds = subscribe(zeds, (value) => $zeds = value);
  for (let i = 0; i < 100; i++) {
    $zeds.push(0);
    points.push({
  ...
  return `${peopleHere && testData ? `<svg style="${"width:" + escape(width2)}"${add_attribute("width", width2, 0)}${add_attribute("height", height, 0)} viewBox="${escape(-width2 / 6) + "\n  " + escape(-height / 2) + "\n  " + escape(width2 / 3) + "\n  " + escape(h_orig)}"><rect${add_attribute("x", -width2 / 2, 0)}${add_attribute("y", -height / 2, 0)}${add_attribute("width", width2, 0)}${add_attribute("height", height, 0)} fill="${"#40826D"}" fill-opacity="${"0.7"}"></rect><g transform="${"scale(" + escape(width2 / 1500) + ")"}">${each(newPitch, (params) => {
    return `<line${spread([
      ...
      { stroke: "white" },
      {
        "stroke-width": escape_attribute_value(1 / params.order * 3)
      },
      { "stroke-opacity": "1" },
      {
        visibility: escape_attribute_value(params.order > pitches ? "hidden" : "visible")
      }
    ], {})}></line>`;
  })}${each(points, (point, i) => {
    return `
      <g class="${"shadow"}" transform="${"translate(" + escape(point.x - 14) + ", " + escape(point.y - 25.5) + ") scale(" + escape(6 * point.scale) + ")"}"><path${add_attribute("d", folk[i % 45].shadow, 0)} fill="${"#40826D"}" transform="${"rotate(-10 50 100) translate(15 -5) skewX(40) scale(1 0.5)"}"${add_attribute("visibility", point.order > people + 2 || $zeds[i] != 0 ? "hidden" : "visible", 0)}></path></g>`;
  })}${each(points, (point, i) => {
    return `
...
const Pyramid = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$unsubscribe_comparisonStore = subscribe(comparisonStore, (value) => $comparisonStore = value);
  let bars;
    ...
    $$bindings.la_change_divided(la_change_divided);
  {
    {
      bars = [];
      let ext = extent$1($pyramidStore[0].concat($pyramidStore[1]));
  ...
  return `${request == "bars" ? `<g transform="${"scale(" + escape(f * 18 / height) + ") translate(0," + escape(padding / 2) + ")"}">${each(bars, (props) => {
    return `<rect${spread([escape_object(props), { rx: "6" }], {})}></rect>
      <text${add_attribute("x", props.x + f, 0)}${add_attribute("y", props.y + f / 2 + 2, 0)} text-anchor="${"middle"}" fill="${"white"}" font-size="${"18"}">${escape(props.agegroup)}</text>`;
  })}<text${add_attribute("x", width2 / 2 - 20 - f / 2, 0)}${add_attribute("y", 18.5 * f + 2 + f, 0)} fill="${"white"}" text-anchor="${"end"}" font-size="${"18"}" font-weight="${"bold"}">female
    </text><text${add_attribute("x", width2 / 2 + 2 * f + 28 - f / 2, 0)}${add_attribute("y", 18.5 * f + 2 + f, 0)} fill="${"white"}" text-anchor="${"start"}" font-size="${"18"}" font-weight="${"bold"}">male
    </text></g>` : ``}
...
const App$1 = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  do {
    $$settled = true;
    animation && change(animation);
    $$rendered = `${animation ? `${validate_component(Pyramid, "Pyramid").$$render($$result, {
      height,
      padding,
      width: width2,
      w,
  ...
  ${validate_component(Header, "Header").$$render($$result, {
	...
	${each(story, (chunk, i) => {
      return `${chunk.type === "Scroller" ? `${validate_component(Scroller, "Scroller").$$render($$result, {
        ...
        foreground: () => {
          return `<div slot="${"foreground"}">${each(chunk.foreground, (section) => {
        ...
        background: () => {
          return `<div slot="${"background"}">${validate_component(Component, "Component").$$render($$result, {
            progress,
            offset,
            index,
            count,
	  ...
	  ${chunk.type === "Filler" ? `${validate_component(Filler, "Filler").$$render($$result, {
        theme: "lightblue",
        short: true,
        wide: true,
        center: false
